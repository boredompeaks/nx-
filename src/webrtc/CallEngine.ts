...
export class CallEngine {
  ...
  private validateOptions(options: CallEngineOptions): void {
    if (!options) {
    ...
    for (const field of required) {
      if (!options[field as keyof CallEngineOptions]) {
  ...
  private configureScreenShareEncoding(sender: RTCRtpSender): void {
    try {
      const params = sender.getParameters();
      if (!params.encodings || params.encodings.length === 0) {
        params.encodings = [{}];
  ...
  private async cleanup(): Promise<void> {
    ...
    if ((this.signaling as any) && typeof (this.signaling as any).disconnect === 'function') {
      await (this.signaling as any).disconnect();
    }
