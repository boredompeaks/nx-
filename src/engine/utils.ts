...
import { ValidationResult, WebRTCError } from './types';
 ...
 */
export function validateMediaConstraints(constraints: MediaStreamConstraints): ValidationResult {
  const errors: string[] = [];
    ...
    if (constraints.video && typeof constraints.video === 'object') {
      ...
      if (videoConstraints.width !== undefined) {
        if (typeof videoConstraints.width === 'number' && videoConstraints.width <= 0) {
          ...
          const width = videoConstraints.width as any;
      ...
      if (videoConstraints.height !== undefined) {
        if (typeof videoConstraints.height === 'number' && videoConstraints.height <= 0) {
          ...
          const height = videoConstraints.height as any;
      ...
      if (videoConstraints.frameRate !== undefined) {
        if (typeof videoConstraints.frameRate === 'number' && videoConstraints.frameRate <= 0) {
          ...
          const frameRate = videoConstraints.frameRate as any;
 ...
 */
export function validateCallEngineOptions(options: any): ValidationResult {
  const errors: string[] = [];
  ...
  if (!Array.isArray(options.turnServers)) {
    ...
    options.turnServers.forEach((server: any, index: number) => {
