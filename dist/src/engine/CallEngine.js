...
import { VIDEO_QUALITY_PRESETS, RECONNECT_MAX_ATTEMPTS, RECONNECT_BASE_DELAY, RECONNECT_MAX_DELAY, BITRATE_ADAPTATION_INTERVAL, PACKET_LOSS_THRESHOLD, RTT_THRESHOLD, BANDWIDTH_SAFETY_MARGIN, MAX_CONNECTIONS_PER_ROOM, DEFAULT_ICE_SERVERS } from './constants';
import { validateMediaConstraints, cleanupMediaStream, cleanupPeerConnection, validateCallEngineOptions, deepClone } from './utils';
export class CallEngine {
    constructor(options) {
        ...
        this.connectionValidation = new Map();
        // Validate options
        const validation = validateCallEngineOptions(options);
        if (!validation.isValid) {
            throw new WebRTCError(`Invalid CallEngine options: ${validation.errors.join(', ')}`, 'INVALID_OPTIONS', 'CallEngine.constructor');
        }
        this.options = {
            videoQuality: 'medium',
            enableSVC: true,
    ...
    async init() {
        ...
        try {
            ...
            const webrtcError = error instanceof WebRTCError ? error : new WebRTCError(`Initialization failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'INITIALIZATION_FAILED', 'CallEngine.init', error instanceof Error ? error : undefined);
    ...
    async startLocalMedia(constraints) {
        try {
            ...
            if (!validation.isValid) {
                throw new WebRTCError(`Invalid media constraints: ${validation.errors.join(', ')}`, 'INVALID_CONSTRAINTS', 'CallEngine.startLocalMedia');
            }
            ...
            const webrtcError = error instanceof WebRTCError ? error : new WebRTCError(`Failed to start local media: ${error instanceof Error ? error.message : 'Unknown error'}`, 'MEDIA_START_FAILED', 'CallEngine.startLocalMedia', error instanceof Error ? error : undefined);
            this.emit('error', { error: webrtcError, context: 'getUserMedia' });
            throw webrtcError;
        }
    }
    enhanceConstraints(constraints, preset) {
    ...
    async addLocalStreamToPeerConnections() {
        if (!this.localStream)
            return;
    ...
    async startScreenShare(options) {
        try {
            ...
            const webrtcError = error instanceof WebRTCError ? error : new WebRTCError(`Failed to start screen share: ${error instanceof Error ? error.message : 'Unknown error'}`, 'SCREEN_SHARE_FAILED', 'CallEngine.startScreenShare', error instanceof Error ? error : undefined);
            this.emit('error', { error: webrtcError, context: 'getDisplayMedia' });
            throw webrtcError;
        }
    }
    async addScreenStreamToPeerConnections() {
    ...
    configureScreenTrackEncoding(sender) {
        try {
            const params = sender.getParameters();
            if (!params.encodings || params.encodings.length === 0) {
                params.encodings = [{}];
    ...
    on(event, handler) {
        if (typeof handler !== 'function') {
            throw new WebRTCError('Event handler must be a function', 'INVALID_HANDLER', 'CallEngine.on');
        }
    ...
    async handleSignal(signal) {
        try {
            ...
            const webrtcError = error instanceof WebRTCError ? error : new WebRTCError(`Failed to handle signal ${signal.type}: ${error instanceof Error ? error.message : 'Unknown error'}`, 'SIGNAL_HANDLING_FAILED', `handleSignal:${signal.type}`, error instanceof Error ? error : undefined);
    ...
    async createPeerConnection(peerId, polite) {
        ...
        if (this.peerConnections.size >= MAX_CONNECTIONS_PER_ROOM) {
            throw new WebRTCError(`Maximum connections per room reached: ${MAX_CONNECTIONS_PER_ROOM}`, 'CONNECTION_LIMIT_REACHED', 'CallEngine.createPeerConnection');
        }
        try {
            ...
            const webrtcError = error instanceof WebRTCError ? error : new WebRTCError(`Failed to create peer connection: ${error instanceof Error ? error.message : 'Unknown error'}`, 'PEER_CONNECTION_CREATION_FAILED', 'CallEngine.createPeerConnection', error instanceof Error ? error : undefined);
            throw webrtcError;
        }
    }
    setupPeerConnectionHandlers(pc, peerId, polite) {
        ...
        pc.onnegotiationneeded = async () => {
            try {
                ...
                this.emit('error', { error: error, context: 'negotiation' });
            }
            finally {
    ...
    async handleAnswer(signal) {
        ...
        try {
            ...
            this.emit('error', { error: error, context: 'handleAnswer' });
        }
    }
    async handleIceCandidate(signal) {
        ...
        try {
            ...
            if (pc.remoteDescription) {
                ...
                this.emit('error', { error: error, context: 'addIceCandidate' });
            }
        }
    }
    ...
    async triggerNegotiation(peerId) {
        ...
        try {
            ...
            this.emit('error', { error: error, context: 'triggerNegotiation' });
        }
    }
    isPolite(peerId) {
    ...
    async restartIce(peerId) {
        ...
        try {
            ...
            this.emit('error', { error: error, context: 'iceRestart' });
        }
    }
