...
import { errorHandler, asyncHandler, requestTimeout, NotFoundError } from './errorHandler.js';
const app = express();
// Security middleware
app.use(helmet({
...
app.post('/chats', validateInput(chatValidation), asyncHandler(async (req, res) => {
...
app.post('/messages', validateInput(messageValidation), asyncHandler(async (req, res) => {
...
app.post('/reactions', validateInput(reactionValidation), asyncHandler(async (req, res) => {
    ...
    const existingReaction = reactions.find(r => r.message_id === req.body.message_id &&
...
app.post('/read-receipts', validateInput(readReceiptValidation), asyncHandler(async (req, res) => {
    // Check if message exists
    if (!messages.has(req.body.message_id)) {
        throw new NotFoundError('Message', req.body.message_id);
    }
    // Check for existing read receipt
    const existingReceipt = receipts.find(r => r.message_id === req.body.message_id && r.user_id === req.body.user_id);
    if (existingReceipt) {
...
app.post('/typing', validateInput(typingValidation), asyncHandler(async (req, res) => {
    // Check if chat exists
    if (!chats.has(req.body.chat_id)) {
        throw new NotFoundError('Chat', req.body.chat_id);
    }
...
app.post('/presence', validateInput(presenceValidation), asyncHandler(async (req, res) => {
    const p = {
        user_id: req.body.user_id,
