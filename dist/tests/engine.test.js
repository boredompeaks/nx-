...
import { validateMediaConstraints, validateCallEngineOptions, cleanupMediaStream, cleanupPeerConnection, exponentialBackoff, probeServerLatency, generateSecureId, deepClone, debounce } from '../src/engine/utils';
...
describe('WebRTC Engine Tests', () => {
    ...
    describe('CallEngine', () => {
        ...
        it('should handle initialization errors gracefully', async () => {
            vi.spyOn(SignalingClient.prototype, 'connect').mockRejectedValue(new WebRTCError('Connection failed', 'CONNECTION_FAILED', 'SignalingClient.connect'));
            engine = new CallEngine(mockOptions);
        ...
        it('should handle media constraints validation errors', async () => {
            ...
            await expect(engine.startLocalMedia(invalidConstraints)).rejects.toThrow(WebRTCError);
        });
    ...
    describe('SignalingClient', () => {
        ...
        it('should create SignalingClient with valid options', () => {
            expect(() => {
                client = new SignalingClient(mockSignalingOptions.supabaseUrl, mockSignalingOptions.supabaseKey, mockSignalingOptions.roomId, mockSignalingOptions.userId);
            }).not.toThrow();
        });
        it('should throw error with invalid options', () => {
            expect(() => {
    ...
    describe('StatsMonitor', () => {
        ...
        it('should throw error with invalid peer connection', () => {
            expect(() => {
                new StatsMonitor(null, mockOnUpdate);
            }).toThrow(WebRTCError);
        });
        it('should throw error with invalid callback', () => {
            expect(() => {
                new StatsMonitor(mockPeerConnection, null);
            }).toThrow(WebRTCError);
        });
    ...
    describe('Integration Tests', () => {
        ...
        it('should handle error recovery', async () => {
            ...
            try {
                await engine.startLocalMedia({ video: { width: -1 } });
            }
            catch (error) {
                expect(error).toBeInstanceOf(WebRTCError);
            }
