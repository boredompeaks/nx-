...
import { CallEngine, SignalingClient, TurnSelector, AudioDucker, StatsMonitor, createCallEngine, isWebRTCSupported, getWebRTCCapabilities } from '../src/webrtc';
...
global.RTCPeerConnection = (_a = class RTCPeerConnection {
        constructor() {
            ...
            this.createOffer = vi.fn(() => Promise.resolve({ type: 'offer', sdp: 'test' }));
...
describe('WebRTC Engine - Enterprise Grade Tests', () => {
    ...
    describe('CallEngine', () => {
        ...
        it('should initialize successfully', async () => {
            ...
            vi.spyOn(engine, 'signaling', 'get').mockReturnValue({
        ...
        it('should toggle mute successfully', async () => {
            ...
            vi.spyOn(engine, 'localStream', 'get').mockReturnValue(mockStream);
    ...
    describe('SignalingClient', () => {
        ...
        it('should create SignalingClient with valid config', () => {
            client = new SignalingClient(mockConfig.supabaseUrl, mockConfig.supabaseKey, mockConfig.roomId, mockConfig.userId);
        ...
        it('should handle connection errors gracefully', async () => {
            ...
            const testClient = new SignalingClient(mockConfig.supabaseUrl, mockConfig.supabaseKey, mockConfig.roomId, mockConfig.userId);
    ...
    describe('AudioDucker', () => {
        ...
        it('should throw error with invalid local stream', async () => {
            await expect(ducker.initialize(null)).rejects.toThrow('Local stream is required');
        });
        it('should handle initialization errors gracefully', async () => {
        ...
        it('should handle cleanup errors gracefully', async () => {
            ...
            await ducker.initialize(mockStream);
            // Mock cleanup error
            const mockContext = new global.AudioContext();
    ...
    describe('StatsMonitor', () => {
        ...
        it('should throw error with invalid peer connection', () => {
            expect(() => {
                new StatsMonitor(null, mockCallback);
        ...
        it('should throw error with invalid callback', () => {
            expect(() => {
                new StatsMonitor(mockPC, 'invalid');
    ...
    describe('Utility Functions', () => {
        describe('validateMediaConstraints', () => {
            ...
            it('should reject invalid constraints', () => {
                ...
                expect(validateMediaConstraints(null)).toBe(false);
    ...
    describe('Error Handling and Edge Cases', () => {
        it('should handle race conditions in peer connection creation', async () => {
            ...
            vi.spyOn(engine, 'turnSelector', 'get').mockReturnValue({
            ...
            mockRTCPC.generateCertificate = vi.fn(() => Promise.resolve({}));
            global.RTCPeerConnection = mockRTCPC;
            // Simulate concurrent peer connection creation
            const promises = [
                engine.createPeerConnection('peer1', true),
                engine.createPeerConnection('peer1', false),
            ];
        ...
        it('should handle maximum peer limit', async () => {
            ...
            vi.spyOn(engine, 'peerConnections', 'get').mockReturnValue(mockMap);
            await expect(engine.createPeerConnection('peer51', true))
        ...
        it('should handle global error limits', async () => {
            ...
            for (let i = 0; i < 15; i++) {
                engine.handleGlobalError(new Error(`Error ${i}`));
            }
    ...
    describe('Performance and Memory Tests', () => {
        it('should handle large numbers of peer connections efficiently', async () => {
            ...
            const promises = [];
            for (let i = 0; i < 10; i++) {
                promises.push(engine.createPeerConnection(`peer${i}`, true));
            }
            await Promise.all(promises);
            const endTime = Date.now();
            // Should complete within reasonable time
            expect(endTime - startTime).toBeLessThan(5000);
            expect(engine.peerConnections.size).toBe(10);
        });
        it('should cleanup resources properly', async () => {
            ...
            await engine.init();
            // Create some state
            await engine.createPeerConnection('peer1', true);
            const mockStream = new MediaStream();
            vi.spyOn(engine, 'localStream', 'get').mockReturnValue(mockStream);
            // Cleanup
            await engine.endCall();
            expect(engine.peerConnections.size).toBe(0);
