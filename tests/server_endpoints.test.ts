...
function makeApp(){
  ...
  app.post('/typing',(req,res)=>{ const t={chat_id:req.body.chat_id,user_id:req.body.user_id,is_typing:!!req.body.is_typing,updated_at:Date.now()}; const i=typing.findIndex(x=>x.chat_id===t.chat_id&&x.user_id===t.user_id); if(i>=0) typing[i]=t; else typing.push(t); res.json(t); });
  app.get('/typing',(req,res)=>{ res.json(typing.filter(t=>t.chat_id===req.query.chat_id)); });
  app.post('/presence',(req,res)=>{ const p={user_id:req.body.user_id,status:req.body.status,updated_at:Date.now()}; const i=presence.findIndex(x=>x.user_id===p.user_id); if(i>=0) presence[i]=p; else presence.push(p); res.json(p); });
